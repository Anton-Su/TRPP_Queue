...
async def process_group(message: types.Message, state: FSMContext):
    ...
    if not group_exist:
        await message.answer(f"⚠ Ошибка: группы «{message.text}» не существует, проверьте корректность шифра (дефисы тоже считаются). Также можно создать пользовательскую группу с помощью /add_group название (но расписание пар не будет автоматически добавляться). ", reply_markup=kbnotregister)
        return
